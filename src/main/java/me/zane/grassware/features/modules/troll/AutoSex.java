...
import me.zane.grassware.util.ColorUtil;
...
class AutoSex
        extends Module {
    float dildoProgess = 0.0f;
    public AutoSex( ) {
        super ( "AutoDildo" , "fucking horny" , Category.TROLL , false , false , false );
    }
    ...
    void onRender3D ( Render3DEvent render3DEvent ) {
        dildoProgess += 0.1;
        for (Object e : Util.mc.world.loadedEntityList) {
            if ( ! ( e instanceof EntityPlayer ) ) continue;
            RenderManager renderManager = Minecraft.getMinecraft ( ).getRenderManager ( );
            EntityPlayer entityPlayer = (EntityPlayer) e;
            double d = entityPlayer.lastTickPosX + ( entityPlayer.posX - entityPlayer.lastTickPosX ) * (double) Util.mc.timer.renderPartialTicks;
            double d2 = d - renderManager.renderPosX;
            double d3 = entityPlayer.lastTickPosY + ( entityPlayer.posY - entityPlayer.lastTickPosY ) * (double) Util.mc.timer.renderPartialTicks;
            double d4 = d3 - renderManager.renderPosY;
            double d5 = entityPlayer.lastTickPosZ + ( entityPlayer.posZ - entityPlayer.lastTickPosZ ) * (double) Util.mc.timer.renderPartialTicks;
    ...
    void esp ( EntityPlayer entityPlayer , double d , double d2 , double d3 ) {
        ...
        GL11.glTranslated ( 0.0 , 0.0 , -4.075f + dildoProgess );
        Cylinder cylinder = new Cylinder ( );
        cylinder.setDrawStyle ( 100013 );
        cylinder.draw ( 0.1f , 0.11f , 1.0f , 25 , 20 );
        GL11.glColor4f ( (ColorUtil.rainbow(50).getRed() / 255.0f) , (ColorUtil.rainbow(50).getGreen() / 255.0f) , (ColorUtil.rainbow(50).getBlue() / 255.0f) , 1.0f );
        GL11.glTranslated ( 0.0 , 0.0 , - 0.12500000298023223 + (dildoProgess / 14) );
