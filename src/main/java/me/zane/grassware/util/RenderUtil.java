...
import me.zane.grassware.features.gui.font.CustomFont;
import me.zane.grassware.features.modules.player.PacketMine;
...
public class RenderUtil
    ...
    private final CustomFont customFont = new CustomFont(new Font("Arial", Font.BOLD | Font.ITALIC, 23), true, false);
    ...
    public void drawText(final BlockPos pos, final String text, final Color color) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-(Quantum.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
    ...
    public static void gradientBox(final BlockPos pos, final Color color, final float lineWidth, final boolean outline, final boolean box, final int boxAlpha, final boolean speedmine) {
        ...
        if (RenderUtil.camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + mc.getRenderManager().viewerPosX, bb.minY + mc.getRenderManager().viewerPosY, bb.minZ + mc.getRenderManager().viewerPosZ, bb.maxX + mc.getRenderManager().viewerPosX, bb.maxY + mc.getRenderManager().viewerPosY, bb.maxZ + mc.getRenderManager().viewerPosZ))) {
            ...
            final double percentage = mc.playerController.curBlockDamageMP;
            float speedmineThing = PacketMine.getInstance().timer.getPassedTimeMs() / 1000.0f / (PacketMine.getInstance().breakTime * Quantum.serverManager.getTpsFactor());
            speedmineThing = (Math.min(speedmineThing, 1.0f));
            double minX;
            double minY;
            double minZ;
            double maxX;
    ...
    public static void drawText(BlockPos pos, String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-((double) Quantum.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Quantum.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
